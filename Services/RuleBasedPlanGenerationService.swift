...
final class RuleBasedPlanGenerationService {
    func generateNextWorkout(
        ...
        let plannedExercises = selectedExercises.map { exercise in
            plannedExercise(
                for: exercise,
    ...
    private func plannedExercise(
        ...
        if isFatigued {
            return PlannedExercise(
                exercise: exercise,
                targetSets: max(previousLog.sets.count - 1, ExerciseProgressionRule.minimumTargetSets),
                targetReps: max(Int(averageReps.rounded()), 1),
                suggestedWeightInKilograms: roundedWeight(averageWeight * ExerciseProgressionRule.fatigueLoadMultiplier),
        ...
        if isHardButCompleted {
            return PlannedExercise(
                ...
                targetReps: max(Int(averageReps.rounded()), 1),
                suggestedWeightInKilograms: roundedWeight(averageWeight),
                targetRPE: ExerciseProgressionRule.defaultTargetRPE,
                targetRIR: ExerciseProgressionRule.defaultTargetRIR
            )
        }
        ...
        return PlannedExercise(
            ...
            targetSets: previousLog.sets.count,
            targetReps: max(Int(averageReps.rounded()), 1),
            suggestedWeightInKilograms: roundedWeight(progressedWeight),
            targetRPE: ExerciseProgressionRule.defaultTargetRPE,
            targetRIR: ExerciseProgressionRule.defaultTargetRIR
        )
    }
